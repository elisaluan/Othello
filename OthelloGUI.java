...
public class OthelloGUI {
   ...
   private JTextField[] playerPieces;
...
// createInfoPanel
   ...
   // Create the array of textfield for players' score
     ...
     // Create a panel for the piece counter
      JPanel piecePanel = new JPanel();
      piecePanel.setBackground(background);
   
   // Create the label to display "PIECE COUNTER" heading
      JLabel pieceLabel = new JLabel ("PIECE COUNTER", JLabel.CENTER);
   ...
   // Create JLabels for players
      JLabel[] playerLabel1 = new JLabel[NUMPLAYER];
      for (int i = 0; i < NUMPLAYER; i++) {
         playerLabel1[i] = new JLabel(playerIcon[i]);
      }
   
   // Create the array of textfield for players' pieces
      playerPieces = new JTextField[NUMPLAYER];
      for (int i = 0; i < NUMPLAYER; i++) {
         playerPieces[i] = new JTextField();
         playerPieces[i].setFont(regularFont);
         playerPieces[i].setText("0");
         playerPieces[i].setEditable(false);
         playerPieces[i].setHorizontalAlignment (JTextField.CENTER);
         playerPieces[i].setPreferredSize (new Dimension (INFOPANEWIDTH - PIECESIZE - 10, 30));
         playerPieces[i].setBackground(background);
         playerPieces[i].setForeground(Color.white);
      }
   
      piecePanel.add(pieceLabel);
      for (int i = 0; i < NUMPLAYER; i++) {
         piecePanel.add(playerLabel1[i]);
         piecePanel.add(playerPieces[i]);
      }
...
/**
* Display the amount of pieces on the textfield of the corresponding player
*
* @param player the player whose pieces are to be displayed
* @param piece the pieces to be displayed
*/
   public void setPieceCount(int player, int pieces) {
      playerPieces[player].setText(pieces+"");
   }
